...
public class PuzzleBoard extends State {
    ...
    public State next(Action action) {
        ...
        int oldDist = Math.abs(newRow - targetRow) + Math.abs(newCol - targetCol);
        int newDist = Math.abs(zeroRow - targetRow) + Math.abs(zeroCol - targetCol);

        newState.setManhattanDistance(manhattanDistance - oldDist + newDist);

        // ? 错位数增量更新
        int v = 0;
        if (val == zeroRow * size + zeroCol + 1) v--; // 移动到目标位置
        if (val == newRow * size + newCol + 1) v++;   // 离开目标位置
    ...
    * */
    ...
    public int getDataBaseDistance663() {
        // 确保数据库已初始化
        if (DataBaseCreate.Table == null) {
            DataBaseCreate.initialize(blocks_663);
        }

        try {
            int dbDist = DataBaseCreate.getDistance(puzzle_board, blocks_663);
            int lcDist = getLinearConflictDistance();
            return Math.max(dbDist, lcDist);
        } catch (Exception e) {
            System.err.println("数据库查询出错，使用线性冲突距离: " + e.getMessage());
            return getLinearConflictDistance();
        }
    }
}
